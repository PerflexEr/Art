  ...
  txtInputs.forEach(input => {
    input.addEventListener("input", e => {
      ...
      if (!inputValue.match(/^[а-яё0-9\s]+$/i)) {
        e.target.value = inputValue.replace(/[^а-яё0-9\s]/gi, "");
      }
    });
  });
};
...
const modals = () => {
  ...
  function showModalByTime(modalSelector, timeToShow) {
    setTimeout(() => {
      let display;
...
const picture = imgSelector => {
  ...
  function showImg(block) {
    ...
    const newSrc = srcParts[0] + "-1." + srcParts[1];
    img.src = newSrc;
  ...
  function hideImg(block) {
    const img = block.querySelector("img");
    const srcParts = img.src.split("-1.");
    const newSrc = srcParts[0] + "." + srcParts[1];
    img.src = newSrc;
    block.querySelectorAll("p:not(.sizes-hit)").forEach(p => {
